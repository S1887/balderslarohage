...
import MultiplicationDashboard from './MultiplicationDashboard';
import ModeSelection from './ModeSelection';

const MultiplicationGame = ({ onBack }) => {
    const { user, addXP, saveScore } = useGame();
    ...
    useEffect(() => {
        if (user?.multiplicationBestFixed) {
            setBestScores(prev => ({
                ...prev,
                fixed: user.multiplicationBestFixed
            }));
        }
        if (user?.multiplicationBestTimed) {
            setBestScores(prev => ({
                ...prev,
                timed: user.multiplicationBestTimed
            }));
        }
    }, [user]);
    ...
    const finishGame = () => {
        ...
        saveScore(gameMode, finalScore, timeString, questionsAnswered);
